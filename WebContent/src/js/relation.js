$(document).ready(function(){
    ...
    layui.use(['layer','form','table','upload','element'],function(){
        ...
        form.on('submit(personChose)', function(data){
            console.log('oooo:'+JSON.stringify(data.field))
            var filterData = data.field
            // var filterData = JSON.stringify(data.field);
            // layer.msg(JSON.stringify(data.field));
            $.ajax({
                ...
                success: function(res){
                    ...
                    var tis1 = table.render({
                        ...
                        cols: [[                           
                            ...
                            { field: 'department_name', title: '部门',edit: 'text' },
                            { field: 'user_type', title: '用户类型',edit: 'text'} ,
                        ...
                        if (obj.event === 'del') {
                            layer.confirm('确定要删除吗?', function (index) {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: 'relation/delete',
                                    data: { 'str':JSON.stringify(data.user_id)},
                                    success: function(res){
                                        layer.msg('删除成功');              
                                    },
                                    error:function(res){
